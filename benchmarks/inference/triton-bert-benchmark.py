...
def get_dummy_query(seq_len, mask, task="fill-mask"):
    assert (
        seq_len > 3
    )  # +2 for special tokens such as CLS or SEP tokens (101 and 102 respectively) and +1 for mask
...
def run_benchmark(
    ...
    print(
        f"run_benchmark: {args}, {pipe}, {query}, {seq_len}, {use_triton}, {ds_run}, {task}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--kernel-inject", action="store_true", help="inject kernels on"
    )
    ...
    parser.add_argument(
        "--deepspeed", action="store_true", help="use deepspeed inference"
    )
    parser.add_argument(
        "--task",
        type=str,
        default="fill-mask",
        help="fill-mask or token-classification",
    )
    ...
    for seq_len in seq_lens:
        e2e_latency, model_latency = run_benchmark(
            args,
            pipe=pipe,
            query=None,
            seq_len=seq_len,
            use_triton=args.triton,
            ds_run=args.deepspeed,
            task=args.task,
        )
