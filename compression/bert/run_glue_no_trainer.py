...
def parse_args():
    ...
    parser.add_argument(
        "--deepspeed_config", type=str, default=None, help="deepspeed config"
    )
    parser.add_argument(
        "--save_best_model", action="store_true", help="save best checkpoint model"
    )
    parser.add_argument(
        "--clean_best_model", action="store_true", help="clean the  model"
    )
    parser.add_argument(
        "--distill_method", type=str, default=None, help="knowledage distillation"
    )
    parser.add_argument(
        "--local_rank",
        type=int,
    ...
    parser.add_argument(
        "--eval_step", type=int, default=1000, help="when to eval the model."
    )
...
def main():
    ...
    if layer_reduction_enabled:
        config.num_hidden_layers = ds_config["compression_training"]["layer_reduction"][
            "keep_number_layer"
    ...
    if args.distill_method != "zero_stage":
        ...
        if args.pretrained_dir_teacher is not None:
            teacher_model.load_state_dict(torch.load(args.pretrained_dir_teacher))
    # model inititalization, config,
    if args.deepspeed:
        if quantization_enabled or prune_enabled or layer_reduction_enabled:
            model = init_compression(
                model, args.deepspeed_config, teacher_model=teacher_model
            )  # <==========================================compression argument

    if args.pretrained_dir_student is not None:
        model.load_state_dict(
            torch.load(args.pretrained_dir_student)
        )  # <==========================================add weight to students if users provides difference models
    ...
    if args.max_train_steps is None:
        args.max_train_steps = math.ceil(
    ...
    if teacher_model is not None:
        ...
        teacher_result, _, _, _ = arrange_output(
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            forward_step += 1
            if (
                forward_step % args.eval_step == 0
                or updated_steps == args.max_train_steps
                or step == len(train_dataloader) - 1
            ):
                results = do_eval(
                    args,
                ...
                arrange_out = arrange_output(
                    args.task_name, results, previous_best, best_dev_acc
                )
                stat_history, best_dev_acc, save_model = update_stat_and_print(
                    args,
                    print_rank_0,
                    forward_step,
                    stat_history,
                ...
                if save_model and args.save_best_model:
                    ...
                    save_checkpoint_and_config(
                        args, model, config, tokenizer, ds_config=ds_config
                    )
...
if __name__ == "__main__":
    ...
    print(f"total runtime: {main_end_time-main_start_time}")
