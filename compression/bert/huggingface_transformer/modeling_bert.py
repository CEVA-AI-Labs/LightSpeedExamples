...
from deepspeed.compression.basic_layer import (
    AsymQuantizer,
    LinearLayer_Compress,
    SymQuantizer,
)
from deepspeed.compression.utils import GeneralQuantizer, global_data_dumper
...
def set_id_to_qat_layers(model, full_name):
    ...
    for name, layer in model._modules.items():
        ...
        if isinstance(
            layer, (LinearLayer_Compress, BertSelfAttention, BertSoftmax, BertLayerNorm)
        ):
...
class BertLayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-5) -> None:
        super().__init__()
        # normalized_shape: number of dimensions from the end to be normalized
        if isinstance(normalized_shape, Integral):
            # mypy error: incompatible types in assignment
            normalized_shape = 1  # type: ignore[assignment]
    ...
    def forward(self, input: torch.Tensor, weight=None, bias=None) -> torch.Tensor:
        """
        Applying manually layer normalization for a given input
        @input type: torch.Tensor: input tensor
        ...
        (
            ...
            var_zp,
            var_quantized,
        ) = GeneralQuantizer.calc_quantization_params(data_buffer=var, num_bits=32)
        ...
        one_over_squared_root_var = 1.0 / torch.sqrt(var + self.eps)
        (
            one_over_squared_root_var_scale,
            one_over_squared_root_var_zp,
            one_over_squared_root_var_quantized,
        ) = GeneralQuantizer.calc_quantization_params(
            data_buffer=one_over_squared_root_var, num_bits=16
        )
        global_data_dumper.dump_quantization_data(
            self.scope_name,
            one_over_squared_root_var_scale,
            one_over_squared_root_var_zp,
            one_over_squared_root_var_quantized,
            "layer_norm_one_over_squared_root_var_t12",
        )
        # Normalize the tensor
        input_normalized = (input - mean) * one_over_squared_root_var
...
class BertSoftmax(nn.Module):
    ...
    def forward(self, input: torch.Tensor, dim: int = -1) -> torch.Tensor:
        """
        Calculating manually the softmax value of a given input
        @input type: torch.Tensor: input tensor
        @dim type: int: dimension to calculate the softmax with respect to
        @return: torch.Tensor result
        """
        # calculate input sf
        (
        ...
        global_data_dumper.dump_quantization_data(
            self.scope_name, input_scale, input_zp, input_quantized, "softmax_input_t1"
        )
        max_values = torch.amax(input=input, dim=dim, keepdim=True)
        # subtract maximum value to avoid overflow
        input_sub_max = input - max_values
        # calculate all exponent values
        exp_values = torch.exp(input_sub_max)
        # calculate all exponent sums
        sigmas = torch.sum(input=exp_values, dim=dim, keepdim=True)
        # calculate sf
        (
            sigmas_scale,
            sigmas_zp,
            sigmas_quantized,
        ) = GeneralQuantizer.calc_quantization_params(data_buffer=sigmas, num_bits=16)
        global_data_dumper.dump_quantization_data(
            self.scope_name,
            sigmas_scale,
            sigmas_zp,
        ...
        # final result is the exp_values / subtracted_sigmas
        return exp_values / sigmas
...
class BertSelfAttention(nn.Module):
    ...
    def add_quant_bits(self, quant_dict):
        quant_dict[self.scope_name] = {
            "key_layer_bits": 4,
            "key_layer_groups": "tbd",
            "quary_layer_bits": 4,
            "quary_layer_groups": 1,
            "attension_score_bits": 8,
            "quary_layer_groups": 1,
            "value_layer_bits": 4,
            "quary_layer_groups": 98000,
        }
    ...
    def forward(
        ...
        (
            ...
            query_zp,
            query_quantized,
        ) = GeneralQuantizer.calc_quantization_params(query_layer, 8)
        global_data_dumper.dump_quantization_data(
            self.scope_name, query_scale, query_zp, query_quantized, "query_layer"
        )
        # quantize key
        (
            key_scale,
            key_zp,
            key_quantized,
        ) = GeneralQuantizer.calc_quantization_params(key_layer, 8)
        global_data_dumper.dump_quantization_data(
            self.scope_name, key_scale, key_zp, key_quantized, "key_layer"
        )
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))

        # quantize matmul output manually
        (
            attention_scores_scale,
            attention_scores_zp,
            attention_scores_quantized,
        ) = GeneralQuantizer.calc_quantization_params(attention_scores, 8)
        global_data_dumper.dump_quantization_data(
            self.scope_name,
            attention_scores_scale,
            attention_scores_zp,
        ...
        # old_attention_probs = nn.functional.softmax(attention_scores, dim=-1)
        # manual implementation of the softmax e^x / (sigma e^x)
        ...
        # quantize manually
        # quantize attention_probs
        (
            attention_probs_scale,
            attention_probs_zp,
            attention_probs_quantized,
        ) = GeneralQuantizer.calc_quantization_params(attention_probs, 8)
        global_data_dumper.dump_quantization_data(
            self.scope_name,
            attention_probs_scale,
            attention_probs_zp,
        ...
        # quantize value layer
        (
            value_scale,
            value_zp,
            value_quantized,
        ) = GeneralQuantizer.calc_quantization_params(value_layer, 8)
        global_data_dumper.dump_quantization_data(
            self.scope_name, value_scale, value_zp, value_quantized, "value_layer"
        )
        context_layer = torch.matmul(attention_probs, value_layer)

        # quantize matmul output manually
        (
            context_scores_scale,
            context_scores_zp,
            context_scores_quantized,
        ) = GeneralQuantizer.calc_quantization_params(context_layer, 8)
        global_data_dumper.dump_quantization_data(
            self.scope_name,
            context_scores_scale,
            context_scores_zp,
